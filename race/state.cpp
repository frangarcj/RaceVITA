...
static int state_restore_0x10(SceUID stream);
...
static int state_store(race_state_t *rs)
{
  ...
  int size_of_z80 =
    (uint32_t)(&(RACE_cz80_struc->CycleSup)) - (uint32_t)(&(RACE_cz80_struc->BC));
...
int state_restore(SceUID stream)
{
  /* Note current position (in case of compatibility rewinds */
  long read_pos = sceIoLseek(stream, 0, PSP2_SEEK_CUR);

  /* Read header */
  struct race_state_header rsh;
  if ((sceIoRead(stream,&rsh,sizeof(rsh))) < 1)
    return 0;

  /* Verify state version */
  if (rsh.state_version == 0x10)
  {
    sceIoLseek(stream, read_pos, PSP2_SEEK_SET); /* Rewind and load old format */
    return state_restore_0x10(stream);
  }
  else if (rsh.state_version != CURRENT_SAVE_STATE_VERSION)
    return 0; /* Unsupported version */
  ...
  /* Read state data */
  race_state_t rs;
  if (sceIoRead(stream,&rs,sizeof(rs)) < 1)
    return 0;
...
int state_store(SceUID stream)
{
  ...
  /* Write to file */
  if (sceIoWrite(stream,&rsh, sizeof(rsh)) < 1)
    return 0;
  if (sceIoWrite(stream,&rsh, sizeof(rsh)) < 1)
    return 0;
