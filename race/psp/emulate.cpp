...
{
  { PSP2_CTRL_LTRIGGER | PSP2_CTRL_RTRIGGER, 16 },
  { PSP2_CTRL_START    | PSP2_CTRL_SELECT,   17 },
  { PSP2_CTRL_SELECT   | PSP2_CTRL_LTRIGGER, 18 },
  { PSP2_CTRL_SELECT   | PSP2_CTRL_RTRIGGER, 19 },
  { PSP_CTRL_ANALUP,   0 }, { PSP_CTRL_ANALDOWN,  1 },
  { PSP_CTRL_ANALLEFT, 2 }, { PSP_CTRL_ANALRIGHT, 3 },
  { PSP2_CTRL_UP,   4 }, { PSP2_CTRL_DOWN,  5 },
  { PSP2_CTRL_LEFT, 6 }, { PSP2_CTRL_RIGHT, 7 },
  { PSP2_CTRL_SQUARE, 8 },  { PSP2_CTRL_CROSS,     9 },
  { PSP2_CTRL_CIRCLE, 10 }, { PSP2_CTRL_TRIANGLE, 11 },
  { PSP2_CTRL_LTRIGGER, 12 }, { PSP2_CTRL_RTRIGGER, 13 },
  { PSP2_CTRL_SELECT, 14 }, { PSP2_CTRL_START, 15 }
};
...
static uint32_t TicksPerUpdate;
static uint64_t LastTick;
static uint64_t CurrentTick;
static uint8_t RewindEnabled;
...
void RunEmulation()
{
  ...
  {
    uint32_t code = current_map.button_map[current_mapping->index];
  ...
  uint32_t TicksPerSecond = sceRtcGetTickResolution();
  if (psp_options.update_freq)
  {
...
void UpdateInputState()
{
  ...
  if (pspCtrlPollControls(&pad))
  {
    ...
    {
      uint32_t code = current_map.button_map[current_mapping->index];
      uint8_t  on = (pad.buttons & current_mapping->mask) == current_mapping->mask;
                          ...
                          void *userdata)
{
  ...
  if (!Rewinding)
  {
    sound_update((uint16_t*)buf, length_bytes); //Get sound data
    dac_update((uint16_t*)buf, length_bytes);
  }
  else /* Render silence */
  {
