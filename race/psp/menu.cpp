  ...
  do
  {
    ...
    do
    {
      ...
      case TAB_OPTIONS:
        ...
        pl_menu_select_option_by_value(item, (void*)(UiMetric.OkButton == PSP2_CTRL_CIRCLE));
                                ...
                                uint32_t button_mask)
{
  ...
  if (button_mask & PSP2_CTRL_LTRIGGER)
  {
    TabIndex--;
    do
    {
  ...
  else if (button_mask & PSP2_CTRL_RTRIGGER)
  {
    TabIndex++;
    do
    {
  ...
  else if ((button_mask & (PSP2_CTRL_START | PSP2_CTRL_SELECT))
    == (PSP2_CTRL_START | PSP2_CTRL_SELECT))
  {
...
static int OnSaveStateButtonPress(const PspUiGallery *gallery,
                                  ...
                                  uint32_t button_mask)
{
  if (button_mask & PSP2_CTRL_SQUARE
    || button_mask & PSP2_CTRL_TRIANGLE)
  {
    char *path;
    char caption[32];
    const char *config_name = pl_file_get_filename(CURRENT_GAME);
    ...
    do /* not a real loop; flow control construct */
    {
      ...
      else if (button_mask & PSP2_CTRL_TRIANGLE)
      {
                             ...
                             uint32_t button_mask)
{
  if (uimenu == &ControlUiMenu)
  {
    if (button_mask & PSP2_CTRL_TRIANGLE)
    {
      pl_menu_item *item;
      int i;
  ...
  else
  {
    ...
    case OPTION_CONTROL_MODE:
      UiMetric.OkButton = (!(int)option->value)
                          ? PSP2_CTRL_CROSS : PSP2_CTRL_CIRCLE;
      UiMetric.CancelButton = (!(int)option->value)
                              ? PSP2_CTRL_CIRCLE : PSP2_CTRL_CROSS;
      break;
    case OPTION_ANIMATE:
...
{
  /* Load image */
  PspImage *image = pspImageLoadPng(path);
...
static void psp_discard_alpha(PspImage *image)
{
  ...
  for (i = image->Viewport.Y; i < image->Viewport.Height; i++)
    for (j = image->Viewport.X; j < image->Viewport.Width; j++)
      ((uint16_t*)image->Pixels)[(i * image->Width) + j] |= 0x8000;
}
...
static PspImage* psp_save_state(const char *path, PspImage *icon)
{
  ...
  if (!thumb) { sceIoClose(f); return NULL; }
...
static int psp_save_options()
{
  ...
  pl_ini_set_int(&file, "Menu", "Control Mode",
                 (UiMetric.OkButton == PSP2_CTRL_CIRCLE));
