...
void WriteSoundChip(SoundChip* chip, _uint8_t data)
{
#if !defined(__GP32__) && !defined(TARGET_PSP)
    SDL_LockAudio();
#endif
...
void dac_mixer(uint16_t* stream, int length_bytes)
{
#if !defined(__GP32__) && !defined(TARGET_PSP)
...
_uint8_t blockSound[CHIPBUFFERLENGTH], blockDAC[CHIPBUFFERLENGTH];		// Gets filled with sound data.
...
{
#ifndef __GP32__
    sound_update((uint16_t*)blockSound, len);	//Get sound data
//    memcpy(stream, blockSound, len);        //put it in the buffer
    ...
    {
        //dac_update((uint16_t*)blockDAC, len);	//Get DAC data
        //SDL_MixAudio(stream, blockDAC, len, volume); //mix it to the buffer

        dac_mixer((uint16_t*)stream, len);	//Get DAC data
    }
#endif
}
#endif /* TARGET_PSP */
